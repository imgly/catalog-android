...
// <code-region>
class PhotoTransform(private val activity: AppCompatActivity) : Example(activity) {
    ...
    override fun invoke() {
        ...
        settingsList.config.getAssetMap(CropAspectAsset::class.java).clear().add(
            // For this example, free crop is not added to ensure that the image has a suitable ratio.
        ...
        settingsList.configure<TransformSettings> {
            ...
            // that best matches the input photo. However, you can also force a specific crop ratio.
